...
import type { Boat, BuilderState, DeliveryInfo, PaymentMethod, OrderResult, SemrekScreen } from "./types";
...
export default function SemrekApp({
  ...
  const handleBuilderComplete = (config: BuilderState, price: number, name: string) => {
  ...
  const handlePay = async (method: PaymentMethod, partnerCashbackUsed: number) => {
    if (!delivery) return;
    ...
    if (safeUsed > 0 && onSpendPartnerCashback) onSpendPartnerCashback(safeUsed);
    if (konEarned > 0 && onAddKon) onAddKon(konEarned);
    if (partnerCashbackEarned > 0 && onAddPartnerCashback) onAddPartnerCashback(partnerCashbackEarned);
  ...
  const resetAll = () => {
    setBoatName(""); setBoatPrice(0); setBuilderConfig(null);
    setDelivery(null); setOrder(null); setScreen("home");
  };
  ...
  return (
    <div className="sem">
      <div style={{ position: "sticky", top: 0, zIndex: 10, display: "flex", justifyContent: "space-between", padding: "10px 14px", background: "rgba(10,37,64,0.95)", backdropFilter: "blur(8px)" }}>
      ...
      <div className="sem__content">
        ...
        {screen === "payment" && delivery && (
          <PaymentScreen totalPrice={totalPrice} partnerCardBalance={partnerCardBalance} onPay={handlePay} onBack={() => setScreen("delivery")} />
        )}
        {screen === "success" && order && <SuccessScreen order={order} onClose={resetAll} />}
      </div>
    </div>
  );
}
