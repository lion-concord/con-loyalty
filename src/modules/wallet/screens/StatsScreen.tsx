...
export default function StatsScreen({ onBack }: Props) {
  ...
  const byCategory = EXPENSE_CATEGORIES.map((cat) => {
    const sum = transactions.filter((t) => t.category === cat.id && t.type === "expense").reduce((s, t) => s + t.amount, 0);
    return { ...cat, sum };
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "20px" }}>
        <div style={{ display: "flex", gap: 12, marginBottom: 28 }}>
          <div style={{ flex: 1, background: "linear-gradient(135deg,#059669,#10b981)", borderRadius: 20, padding: "20px", color: "#fff" }}>
            ...
            <div style={{ fontSize: 24, fontWeight: 800, marginTop: 6 }}>{totalIncome.toLocaleString("ru-RU")} ₽</div>
          </div>
          <div style={{ flex: 1, background: "linear-gradient(135deg,#dc2626,#ef4444)", borderRadius: 20, padding: "20px", color: "#fff" }}>
            ...
            <div style={{ fontSize: 24, fontWeight: 800, marginTop: 6 }}>{totalExpense.toLocaleString("ru-RU")} ₽</div>
          </div>
        </div>
        {topCategory && (
            ...
            return (
              <div key={cat.id} style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 600, color: "rgba(200,225,255,0.7)" }}>{cat.sum.toLocaleString("ru-RU")} ₽</span>
                </div>
