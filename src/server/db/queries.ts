...
export function ensureUser(telegramId: number, username?: string): User {
  ...
  if (!user) {
    user = { telegram_id: telegramId, username, kon_balance: 0, created_at: new Date().toISOString() };
...
export function createOrder(orderData: {
  orderNumber: string; userId: number; partner: string; amount: number;
  items: string; konSpent: number; contactName?: string; contactPhone?: string;
  deliveryRegion?: string; deliveryMethod?: string; deliveryAddress?: string;
}): number {
  db.read();
  const id = db.data!.orders.length > 0 ? Math.max(...db.data!.orders.map((o: Order) => o.id)) + 1 : 1;
  const order: Order = {
    id, order_number: orderData.orderNumber, user_id: orderData.userId,
    partner: orderData.partner, amount: orderData.amount, items: orderData.items,
    kon_spent: orderData.konSpent, kon_earned: 0, cashback_rubles: 0, status: 'pending',
    contact_name: orderData.contactName || '', contact_phone: orderData.contactPhone || '',
    delivery_region: orderData.deliveryRegion || '', delivery_method: orderData.deliveryMethod || '',
    delivery_address: orderData.deliveryAddress || '', created_at: new Date().toISOString(),
  };
...
export function markOrderPaid(id: number, konEarned: number, cashbackRubles: number): boolean {
  ...
  if (user) user.kon_balance += konEarned;
  const hId = db.data!.kon_history.length > 0 ? Math.max(...db.data!.kon_history.map((h: KonHistory) => h.id)) + 1 : 1;
  db.data!.kon_history.push({ id: hId, user_id: order.user_id, amount: konEarned, type: 'earn', description: 'Order #' + order.order_number, order_id: order.id, created_at: new Date().toISOString() });
  ...
  if (!pc) {
    ...
    pc = { id: pcId, user_id: order.user_id, partner: order.partner, balance: 0 };
...
export function addKon(userId: number, amount: number, type: string, description?: string, orderId?: number): void {
  db.read();
  const id = db.data!.kon_history.length > 0 ? Math.max(...db.data!.kon_history.map((h: KonHistory) => h.id)) + 1 : 1;
db.data!.kon_history.push({ id, user_id: userId, amount, type, description, order_id: orderId, created_at: new Date().toISOString() });
